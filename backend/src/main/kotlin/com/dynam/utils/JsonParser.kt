...
import com.dynam.repositories.*
import com.dynam.dtos.table.*
...
class JsonParser {
    ...
    private val constantRepository  = ConstantRepository()
    ...
    suspend fun parseJsonFile(filePath: String, namespaceName: String) {
        ...
        // Get or create namespace using repository
        val namespace = namespaceRepository.getOrCreate(namespaceName)
        
        // Get module description and create an entity for the namespace with this description
        val moduleDescription = jsonObject["description"]?.jsonPrimitive?.content ?: ""
        if (moduleDescription.isNotEmpty()) {
            // Get or create a namespace entity using repository
            entityRepository.getOrCreateNamespaceEntity(
    ...
    private suspend fun processClass(classObj: JsonObject, namespaceId: Int) {
        ...
        // Create class entity
        val classEntity = entityRepository.create(
        ...
        classObj["methods"]?.jsonArray?.forEach { methodElement ->
            ...
            val functionEntity = entityRepository.create(
    ...
    private suspend fun processConstant(constantObj: JsonObject, namespaceId: Int) {
        ...
        // Get the namespace from repository
        val currentNamespace = namespaceRepository.getById(namespaceId)
        val namespaceName = currentNamespace?.name ?: "unknown"
        
        // Get or create a namespace entity using repository
        val namespaceEntity = entityRepository.getOrCreateNamespaceEntity(
            namespaceName = namespaceName,
            namespaceId = namespaceId,
